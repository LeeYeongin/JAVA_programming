...
public class Test02 {
	...
	public static void main(String[] args) {
		...
		lst.add(ix);
		lst.add(1, "good");
//		int len3 = (lst.get(0)).length();//오류 > list.get(0).은 Object이기 때문이다. -> type변경시 사용가능
		int len4 = ((String)lst.get(0)).length();
		
		//페이지 468 -  generic 타입<>아네 Type을 학습
		List<String> list = new ArrayList<String>();//크기가 10인 리스트 생성
//		ArrayList<없어도 됨>() -> 이 괄호안에 type선언할 필요 없음
		...
		// list안의 객체들을 set(집합)으로 반환받아옴 -> heap이 아닌 CPU 메모리에 따로 가지고 있음
		Iterator<String> iter = list.iterator();//iterator()는 iterator<String> 객체를 반환 (String 객체들의 집합 = Iterator 객체)
		//480페이지 1번째 문단 
		while (iter.hasNext()) { // 현재 커서 다음에 데이터가 있는가 (return true/false). 커서는 첫 데이터 앞에서부터 시작
			System.out.print(iter.next() + " "); // cursor가 1씩 이동하며 set에서 하나씩 읽어옴
		}
		System.out.println();
